...
import (
	pocconfig "OntologyWithPOC/consensus/poc/config"
	"bytes"
	"crypto/sha256"
	"fmt"
	"hash"
	...
	"OntologyWithPOC/consensus/vbft/config"
	"OntologyWithPOC/core/payload"
	"OntologyWithPOC/core/signature"
	"OntologyWithPOC/core/states"
	"OntologyWithPOC/core/store"
	...
	scommon "OntologyWithPOC/smartcontract/common"
	"OntologyWithPOC/smartcontract/event"
	"OntologyWithPOC/smartcontract/service/native/global_params"
	"OntologyWithPOC/smartcontract/service/native/utils"
	"OntologyWithPOC/smartcontract/service/neovm"
	sstate "OntologyWithPOC/smartcontract/states"
	"OntologyWithPOC/smartcontract/storage"
...
type LedgerStoreImp struct {
	...
	pocPeerInfoheader    map[string]uint32 //pubInfo save pubkey,peerindex
	pocPeerInfoblock     map[string]uint32 //pubInfo save pubkey,peerindex
	lock                 sync.RWMutex
...
func NewLedgerStore(dataDir string, stateHashHeight uint32) (*LedgerStoreImp, error) {
	ledgerStore := &LedgerStoreImp{
		...
		pocPeerInfoheader:    make(map[string]uint32),
		pocPeerInfoblock:     make(map[string]uint32),
...
func (this *LedgerStoreImp) InitLedgerStoreWithGenesisBlock(genesisBlock *types.Block, defaultBookkeeper []keypair.PublicKey) error {
	...
	if consensusType == "vbft" {
		...
		this.lock.Unlock()
	} else if consensusType == "poc" {
		header, err := this.GetHeaderByHash(this.currBlockHash)
		if err != nil {
			return err
		}
		blkInfo, err := pocconfig.PocBlock(header)
		if err != nil {
			return err
		}
		...
		if blkInfo.NewChainConfig != nil {
			...
			Info, err := pocconfig.PocBlock(cfgHeader)
			if err != nil {
				return err
			}
			if Info.NewChainConfig == nil {
		...
		this.lock.Lock()
		this.pocPeerInfoheader = make(map[string]uint32)
		this.pocPeerInfoblock = make(map[string]uint32)
		for _, p := range cfg.Peers {
			this.pocPeerInfoheader[p.ID] = p.Index
			this.pocPeerInfoblock[p.ID] = p.Index
		}
		this.lock.Unlock()
	}
...
func (this *LedgerStoreImp) verifyHeader(header *types.Header, PeerInfo map[string]uint32) (map[string]uint32, error) {
	if header.Height == 0 {
		return PeerInfo, nil
	}
	...
	if err != nil && err != scom.ErrNotFound {
		return PeerInfo, fmt.Errorf("get prev header error %s", err)
	}
	if prevHeader == nil {
		return PeerInfo, fmt.Errorf("cannot find pre header by blockHash %s", prevHeaderHash.ToHexString())
	}

	if prevHeader.Height+1 != header.Height {
		return PeerInfo, fmt.Errorf("block height is incorrect")
	}

	if prevHeader.Timestamp >= header.Timestamp {
		return PeerInfo, fmt.Errorf("block timestamp is incorrect")
	}
	consensusType := strings.ToLower(config.DefConfig.Genesis.ConsensusType)
	if consensusType == "vbft" {
		//check bookkeeppers
		m := len(PeerInfo) - (len(PeerInfo)*6)/7
		if len(header.Bookkeepers) < m {
			return PeerInfo, fmt.Errorf("header Bookkeepers %d more than 6/7 len vbftPeerInfo%d", len(header.Bookkeepers), len(PeerInfo))
		}
		for _, bookkeeper := range header.Bookkeepers {
			pubkey := vconfig.PubkeyID(bookkeeper)
			_, present := PeerInfo[pubkey]
			if !present {
				log.Errorf("invalid pubkey :%v,height:%d", pubkey, header.Height)
				return PeerInfo, fmt.Errorf("invalid pubkey :%v", pubkey)
			}
		}
		hash := header.Hash()
		err = signature.VerifyMultiSignature(hash[:], header.Bookkeepers, m, header.SigData)
		if err != nil {
			log.Errorf("VerifyMultiSignature:%s,Bookkeepers:%d,pubkey:%d,heigh:%d", err, len(header.Bookkeepers), len(PeerInfo), header.Height)
			return PeerInfo, err
		}
		blkInfo, err := vconfig.VbftBlock(header)
		if err != nil {
		...
		if blkInfo.NewChainConfig != nil {
			newpeerInfo := make(map[string]uint32)
			for _, p := range blkInfo.NewChainConfig.Peers {
				newpeerInfo[p.ID] = p.Index
			}
		...
		for _, bookkeeper := range header.Bookkeepers {
			pubkey := pocconfig.PubkeyID(bookkeeper)
			_, present := PeerInfo[pubkey]
			if !present {
				log.Errorf("invalid pubkey :%v,height:%d", pubkey, header.Height)
				return PeerInfo, fmt.Errorf("invalid pubkey :%v", pubkey)
		...
		if prevHeader.NextBookkeeper != address {
			return PeerInfo, fmt.Errorf("bookkeeper address error")
		}
...
func (this *LedgerStoreImp) AddHeader(header *types.Header) error {
	...
	if consensusType == "vbft" {
		...
		this.pocPeerInfoheader, err = this.verifyHeader(header, this.pocPeerInfoheader)
		if err != nil {
			return fmt.Errorf("verifyHeader error %s", err)
		}
	} else {
...
func (this *LedgerStoreImp) SubmitBlock(block *types.Block, result store.ExecuteResult) error {
	...
	if consensusType == "vbft" {
		...
		this.pocPeerInfoblock, err = this.verifyHeader(block.Header, this.pocPeerInfoblock)
		if err != nil {
			return fmt.Errorf("verifyHeader error %s", err)
		}
	} else {
