...
import (
	...
	scommon "OntologyWithPOC/core/store/common"
	"OntologyWithPOC/core/store/overlaydb"
	gov "OntologyWithPOC/smartcontract/service/native/governance"
...
func GetPocConfigInfo(memdb *overlaydb.MemDB) (*config.POCConfig, error) {
	//get governance view
	...
	chainconfig := new(config.POCConfig)
	if preCfg.SetView == goveranceview.View {
		chainconfig = &config.POCConfig{
...
func GetPeersConfig(memdb *overlaydb.MemDB) ([]*config.POCPeerStakeInfo, error) {
	goveranceview, err := GetGovernanceView(memdb)
	if err != nil {
		return nil, err
	}
...
func getChainConfig(memdb *overlaydb.MemDB, blkNum uint32) (*pocconfig.ChainConfig, error) {
	config, err := GetPocConfigInfo(memdb)
	if err != nil {
	...
	cfg, err := pocconfig.GenesisChainConfig(config, peersinfo, goverview.TxHash, blkNum)
	if err != nil {
