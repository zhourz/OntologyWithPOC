...
type PeerPool struct {
	...
	server  *Server
	configs map[uint32]*pocconfig.PeerConfig // peer index to peer
...
func NewPeerPool(maxSize int, server *Server) *PeerPool {
	return &PeerPool{
		maxSize:                maxSize,
		server:                 server,
		configs:                make(map[uint32]*pocconfig.PeerConfig),
...
func (pool *PeerPool) clean() {
	...
	pool.configs = make(map[uint32]*pocconfig.PeerConfig)
...
func (pool *PeerPool) addPeer(config *pocconfig.PeerConfig) error {
	pool.lock.Lock()
	defer pool.lock.Unlock()

	peerPK, err := pocconfig.Pubkey(config.ID)
	if err != nil {
...
func (pool *PeerPool) getPeer(idx uint32) *Peer {
	...
	if peer != nil {
		if peer.PubKey == nil {
			peer.PubKey, _ = pocconfig.Pubkey(pool.configs[idx].ID)
		}
		return peer
	}
