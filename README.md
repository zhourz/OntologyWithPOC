...
[![GoDoc](https://godoc.org/OntologyWithPOC?status.svg)](https://godoc.org/OntologyWithPOC)
[![Go Report Card](https://goreportcard.com/badge/OntologyWithPOC)](https://goreportcard.com/report/OntologyWithPOC)
...
New features are still being rapidly developed, therefore the master branch may be unstable. Stable versions can be found in the [releases section](https://OntologyWithPOC/releases).
...
- Downloading a specific version from the [release section](https://OntologyWithPOC/releases).
...
```
$ git clone https://OntologyWithPOC.git
```
or
```
$ go get OntologyWithPOC
```

2) Fetch the dependent third party packages with [Glide](https://glide.sh):

```
$ cd $GOPATH/src/OntologyWithPOC
$ glide install
```
...
The Ontology CLI can run nodes for the MainNet, TestNet and local PrivateNet. Check out the [Ontology CLI user guide](https://OntologyWithPOC/blob/master/docs/specifications/cli_user_guide.md) for a full list of commands.
...
Contributors to Ontology are very welcome! Before beginning, please take a look at our [contributing guidelines](CONTRIBUTING.md). You can open an issue by [clicking here](https://OntologyWithPOC/issues/new).
